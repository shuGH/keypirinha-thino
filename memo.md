...
| Task |              タスク               |                                      内容                                       |                  備考                  |
| ---- | --------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------- |
| 1    | 設定読み込みの整理                | `Section`、`enabled`、`ensure_blank_line`、`[main]` / `[custom/*]` の扱いを整理 | まず設定モデルを安定させる             |
| 2    | `on_suggest` 実装                 | 入力メモから suggestion を生成し、section index と memo を保持                  | メモ整形は一旦スタブ                   |
| 3    | `on_execute` 実装                 | `Append` / `Append and Open` の実行分岐を作る                                   | CLI 呼び出しは一旦スタブ               |
| 4    | メモ整形ヘルパー追加              | `memo_format` に `{memo}` と `strftime` を適用する                              | 整形結果はデバッグ出力で確認           |
